...
const CURATED = {
  ...
  "pc-audit": { rank: 3, blurb: "Reads what Windows already measured about itself, and prints \"not measured\" where the rest of the genre guesses." },
  discern:    { rank: 4, blurb: "Wilson intervals and paired McNemar, so two eval runs are not called apart when the difference is noise." },
  depgraph:   { rank: 5, blurb: "One recursive CTE walks the whole npm graph, so a package reachable forty ways is still counted once." },
  decoder:    { rank: 6, blurb: "Paste a token, a hash, a timestamp. It names it, opens it, and keeps opening what it finds inside." },
  tokenview:  { rank: 7, blurb: "Watch a sentence break into the pieces a model reads, then watch its meaning land on a map." },
  hikari:     { rank: 8, blurb: "Desktop widgets that are a folder and two files. A capability has to be asked for, and is refused by default." },
  yozora:     { rank: 9, blurb: "Fifteen palettes, a type scale and the texture rules, as one small system every page here runs on." },
  "skill-radar": { rank: 10, blurb: "Which agent skill actually fires for a given sentence, and which two are quietly competing." },
  shelfwear:  { rank: 11, blurb: "Reads the files Steam already wrote to tell you what you bought and never launched." },
  "streaming-markdown": { rank: 12, blurb: "Trim a half-arrived markdown frame to the longest valid prefix, so nothing has to be un-rendered." },
  pane:       { rank: 13, blurb: "Drive a Chrome tab from a script. Caching off, fonts waited for, clips clamped, frames matched." },
  "neon-bar": { rank: 14, blurb: "A status bar for Windows, on fifteen palettes. Opaque, because a see-through bar is one a grey wallpaper can erase." },
  "skill-lint": { rank: 15, blurb: "Broken references, colliding triggers and context bloat in an agent's SKILL.md." },
  "agent-skills": { rank: 16, blurb: "The skills themselves, each one written because a specific failure kept happening." },
};
